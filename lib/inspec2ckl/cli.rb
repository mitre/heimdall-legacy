...
class MyCLI < Thor
  ...
  def exec
    ...
    cklist_doc = options[:cklist] ? File.open(options[:cklist]) { |f| Nokogiri::XML(f) } : nil
    ...
    inspec2ckl = Inspec2ckl.new(inspec_json, cklist_doc, title, date)
    File.write(options[:output], inspec2ckl.to_ckl)
  end
